...
export class SeedComics {
  ...
  async seed(): Promise<void> {
    ...
    for (const comicData of comicsData) {
      ...
      // Create comic with stats and categories
      const savedComic = await this.prisma.comic.create({
        data: {
          slug: comicData.slug,
      ...
      this.logger.log(`Created comic: ${savedComic.title} with ${comicCategories.length} categories`);
    }
